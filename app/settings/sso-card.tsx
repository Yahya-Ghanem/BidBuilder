...
export function SsoCard({ isAdmin }: { isAdmin: boolean }) {
  ...
  return (
    <Card className="space-y-4 p-5">
      ...
      {data && (
        <div className="space-y-2 rounded-md border border-[var(--border)] bg-slate-50 p-3 text-xs">
          <p className="font-medium text-slate-600">Register these with your IdP:</p>
      ...
      <Field label={data?.hasCertificate ? "IdP signing certificate (PEM) — leave blank to keep the current one" : "IdP signing certificate (PEM)"}>
        <textarea
          ...
          rows={4}
          className="w-full rounded-md border border-[var(--border)] bg-white px-3 py-2 font-mono text-xs text-slate-700 focus:border-[var(--brand)] focus:outline-none"
        />
      </Field>
