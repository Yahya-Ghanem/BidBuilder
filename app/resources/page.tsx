  ...
  return (
    <Card className="overflow-hidden">
      <div className="flex items-center justify-between border-b border-[var(--border)] px-4 py-2">
        ...
        {canAdd && <Button variant="ghost" className="h-7 px-2 text-sm" onClick={() => setAdding(true)}><Plus className="h-3.5 w-3.5" /> {t("common.add")}</Button>}
      </div>
      ...
      {selected.size > 0 && (
        <div className="flex flex-wrap items-center gap-2 border-b border-[var(--border)] bg-slate-50 px-4 py-2 text-xs">
          ...
          {canEdit && <Button variant="outline" className="h-7 text-sm" disabled={bulk.isPending} onClick={() => runBulk("activate")}><Power className="h-3.5 w-3.5" /> {t("common.activate")}</Button>}
          {canEdit && <Button variant="outline" className="h-7 text-sm" disabled={bulk.isPending} onClick={() => runBulk("deactivate")}><PowerOff className="h-3.5 w-3.5" /> {t("common.deactivate")}</Button>}
          ...
          <button onClick={clear} className="ms-auto text-muted hover:text-slate-600">{t("common.clear")}</button>
        </div>
      )}
      ...
      {isLoading ? <p className="p-4 text-sm text-muted">{t("common.loading")}</p>
        ...
        : (
          ...
          <table className="w-full min-w-[34rem] text-sm">
            ...
            <tbody>
              {rows.map((r) => (
                <tr key={r.id} className={`border-t border-[var(--border)] ${selected.has(r.id) ? "bg-[var(--brand)]/5" : ""} ${r.isActive ? "" : "text-muted"}`}>
                  ...
                  <td className="px-4 py-2 font-mono text-sm">{r.code}{!r.isActive && <span className="ms-1 rounded bg-slate-100 px-1 text-[10px] text-slate-500">{t("resources.inactive")}</span>}</td>
                  ...
                  <td className="px-2 py-2">
                    <div className="flex justify-end gap-1">
                      {canEdit && <button onClick={() => setEditing(r)} className="rounded p-1 text-muted hover:bg-slate-100 hover:text-slate-700"><Pencil className="h-3.5 w-3.5" /></button>}
                      {canDelete && <button onClick={() => { if (confirm(t("resources.deleteConfirm", { code: r.code }))) del.mutate(r.id) }} className="rounded p-1 text-muted hover:bg-rose-50 hover:text-rose-600"><Trash2 className="h-3.5 w-3.5" /></button>}
          ...
          <Field label={`${t(cfg.rateLabelKey)} *`}>
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="flex h-9 w-9 shrink-0 items-center justify-center rounded-md border border-[var(--border)] text-muted transition hover:border-[var(--brand)] hover:text-[var(--brand)] disabled:opacity-40"
              >
                ...
                <>
                  <div className="mb-2 flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-muted">{s.basis}</span>
                    </div>
                  </div>
                  ...
                  {s.suggestedRate === 0 && (
                    <p className="text-muted">{t("resources.aiNone")}</p>
                  )}
                </>
              )
            })()}
