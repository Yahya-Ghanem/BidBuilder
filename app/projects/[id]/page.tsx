...
import type { Project, EstimateSummary, EstimateBreakdown, SectionBreakdown, ItemBreakdown, AssemblyRow, ProjectTeam, GroupOption, MarkupBreakdown, WhatIfResult, ImportResult, CurrencyRates, CostComponentType, ActivityType, ProjectType, Area, AreaRollup, AreaRollupRow, RiskBreakdown, CashFlowMonth, CashFlowProjection } from "@/lib/types"
import { AppShell } from "@/components/app-shell"
import { useAuth } from "@/lib/auth"
import { usePermissions } from "@/lib/permissions"
import { Card, Badge, Button, Input, statusColor } from "@/components/ui"
import { Modal, Field, Select, Textarea } from "@/components/form"
...
function BidLetterModal({ estimateId, bidPrice, currency, onClose }: { estimateId: number; bidPrice: number; currency: string; onClose: () => void }) {
 ...
 * Read-only view of the register with inline add/edit/delete; one-click "apply
 * suggestion onto the Contingency markup". The S-curve renders as a tiny SVG
 * sparkline plus a per-month table — kept compact intentionally; richer
 * visualization belongs to 19.6's split.
 */
function RiskAndCashFlowPanel({ estimateId, currency, risks, suggestedAmount, suggestedPct, cashFlow, canEdit }: {
...
function ifMatchHeaders(qc: ReturnType<typeof useQueryClient>, estimateId: number): HeadersInit {
